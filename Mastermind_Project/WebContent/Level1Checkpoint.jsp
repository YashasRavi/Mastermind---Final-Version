...
<html>
	...
	<body>
		...
		<%
			...
			String conAns_1 = request.getParameter("ConAns1String");
			String connections_1 = request.getParameter("Connections1String");
		...
		<%
			...
			for (int i = 0; i < conAns_1.length(); i++) {
				if (conAns_1.substring(i,i+1).equals("T")) {
					myUser.setConAns1(i, true);
					counter++;
				}
				else {
					myUser.setConAns1(i,false);
				}	
			}
		
			for (int j = 0; j < connections_1.length(); j++) {
				if (connections_1.substring(j,j+1).equals("0")) {
					myUser.setConnections1(j, 0);
				}
				else {
					myUser.setConnections1(j,1);
				}
			}
		
			if (counter == 4) {
				myUser.setTotalPoints(1,10);
			}
			else if (counter == 8) {
				myUser.setTotalPoints(2,10);
			}
			else if (counter == 12) {
				myUser.setTotalPoints(3,10);
			}
			else {
		...
		<div id="buttons"> 
			<button class="myBtn" onclick="location.href = 'Level1.jsp?uId=<%=myUser.getUsername()%>'"> 
			 <h5> Return to Level 1! </h5> 
			</button>
			<br>
			<br>
...
if (<%=a%>) {
	document.querySelector(".userText").innerHTML = "Congrats on finishing Level 1! Level 2 is now unlocked on your profile!";
