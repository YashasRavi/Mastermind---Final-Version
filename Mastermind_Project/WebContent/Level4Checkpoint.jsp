...
<html>
	<head>
	  <title> Mastermind - Checkpoint (Level 4) </title>
	  <meta charset="UTF-8">
	...
	<body>
		...
		<%
			...
			String conAns_1 = request.getParameter("ConAns4String");
			String connections_1 = request.getParameter("Connections4String");
		...
		<%
			...
			for (int i = 0; i < conAns_1.length(); i++) {
				if (conAns_1.substring(i,i+1).equals("T")) {
					myUser.setConAns4(i, true);
					counter++;
				}
				else {
					myUser.setConAns4(i,false);
				}	
			}
		
			for (int j = 0; j < connections_1.length(); j++) {
				if (connections_1.substring(j,j+1).equals("0")) {
					myUser.setConnections4(j, 0);
				}
				else {
					myUser.setConnections4(j,1);
				}
			}
		
			if (counter == 4) {
				myUser.setTotalPoints(10,25);
			}
			else if (counter == 8) {
				myUser.setTotalPoints(11, 25);
			}
			else if (counter == 12) {
				myUser.setTotalPoints(12, 25);
			}
			else {
		...
		<div id="buttons"> 
			<button class="myBtn" onclick="location.href = 'Level4.jsp?uId=<%=myUser.getUsername()%>'"> 
			 <h5> Return to Level 4! </h5> 
			</button>
			<br>
			<br>
...
if (<%= a %>) {
	document.querySelector(".userText").innerHTML = "Congrats on finishing Level 4! Level 5 is now unlocked on your profile!";
