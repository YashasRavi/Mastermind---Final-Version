...
<html>
	<head>
	  <title> Mastermind - Checkpoint (Level 3) </title>
	  <meta charset="UTF-8">
	...
	<body>
		...
		<%
			...
			String conAns_1 = request.getParameter("ConAns3String");
			String connections_1 = request.getParameter("Connections3String");
		...
		<%
			...
			for (int i = 0; i < conAns_1.length(); i++) {
				if (conAns_1.substring(i,i+1).equals("T")) {
					myUser.setConAns3(i, true);
					counter++;
				}
				else {
					myUser.setConAns3(i,false);
				}	
			}
		
			for (int j = 0; j < connections_1.length(); j++) {
				if (connections_1.substring(j,j+1).equals("0")) {
					myUser.setConnections3(j, 0);
				}
				else {
					myUser.setConnections3(j,1);
				}
			}
		
			if (counter == 4) {
				myUser.setTotalPoints(7,20);
			}
			else if (counter == 8) {
				myUser.setTotalPoints(8,20);
			}
			else if (counter == 12) {
				myUser.setTotalPoints(9,20);
			}
			else {
		...
		<div id="buttons"> 
			<button class="myBtn" onclick="location.href = 'Level3.jsp?uId=<%=myUser.getUsername()%>'"> 
			 <h5> Return to Level 3! </h5> 
			</button>
			<br>
			<br>
...
if (<%= a %>) {
	document.querySelector(".userText").innerHTML = "Congrats on finishing Level 3! Level 4 is now unlocked on your profile!";
