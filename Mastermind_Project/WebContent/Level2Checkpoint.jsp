...
<html>
	<head>
	  <title> Mastermind - Checkpoint (Level 2) </title>
	  <meta charset="UTF-8">
	...
	<body>
		...
		<%
			...
			String conAns_1 = request.getParameter("ConAns2String");
			String connections_1 = request.getParameter("Connections2String");
		...
		<%
			...
			for (int i = 0; i < conAns_1.length(); i++) {
				if (conAns_1.substring(i,i+1).equals("T")) {
					myUser.setConAns2(i, true);
					counter++;
				}
				else {
					myUser.setConAns2(i,false);
				}	
			}
		
			for (int j = 0; j < connections_1.length(); j++) {
				if (connections_1.substring(j,j+1).equals("0")) {
					myUser.setConnections2(j, 0);
				}
				else {
					myUser.setConnections2(j,1);
				}
			}
		
			if (counter == 4) {
				myUser.setTotalPoints(4,15);
			}
			else if (counter == 8) {
				myUser.setTotalPoints(5,15);
			}
			else if (counter == 12) {
				myUser.setTotalPoints(6,15);
			}
			else {
		...
		<div id="buttons"> 
			<button class="myBtn" onclick="location.href = 'Level2.jsp?uId=<%=myUser.getUsername()%>'"> 
			 <h5> Return to Level 2! </h5> 
			</button>
			<br>
			<br>
...
if (<%= a %>) {
	document.querySelector(".userText").innerHTML = "Congrats on finishing Level 2! Level 3 is now unlocked on your profile!";
